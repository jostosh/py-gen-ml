...
class Generator(abc.ABC):
    ...
    Subclasses describe themselves to the registry through two class attributes:

    - ``name``: stable identifier used to enable/disable the generator.
    - ``output_suffix``: filename suffix appended to ``<proto>.proto`` when
      emitting Python files. ``None`` for generators that do not emit a per-file
      Python module (e.g. ``InitGenerator``, ``JsonSchemaTaskGenerator``).

    Generators that emit Python types should set ``type_mapper`` to a
    :class:`~py_gen_ml.plugin.type_mapping.TypeMapper` implementation; this is
    how custom generators (LanceDB, torch, etc.) can plug in alternative kind
    tables while reusing the iteration helpers below.
    """

    name: ClassVar[str] = ''
    output_suffix: ClassVar[Optional[str]] = None
    ...
    def _new_python_file(
        ...
        suffix: str,
        *,
        emit_typing_import: bool = False,
        emit_pgml_import: bool = True,
    ) -> protogen.GeneratedFile:
        """Create a new Python output file with the standard banner.

        Subclasses can append additional imports after the banner. ``suffix``
        replaces ``.proto`` in the input filename.
        """
        g = self._gen.new_generated_file(
            file.proto.name.replace('.proto', suffix),
            file.py_import_path,
        )
        g.P('# Autogenerated code. DO NOT EDIT.')
        if emit_typing_import:
            g.P('import typing')
        if emit_pgml_import:
    ...
    def _iter_messages_topo(self, file: protogen.File) -> Iterator[protogen.Message]:
        """Yield messages in topological order (dependencies first)."""
        subgraphs = get_element_subgraphs(file, include_elements={protogen.Kind.ENUM, protogen.Kind.MESSAGE})
        for subgraph in subgraphs:
            for node in networkx.topological_sort(subgraph):
                if isinstance(node, protogen.Message):
                    yield node

    def _iter_enums_topo(self, file: protogen.File) -> Iterator[protogen.Enum]:
        """Yield enums in topological order (dependencies first)."""
        subgraphs = get_element_subgraphs(file, include_elements={protogen.Kind.ENUM, protogen.Kind.MESSAGE})
        for subgraph in subgraphs:
            for node in networkx.topological_sort(subgraph):
                if isinstance(node, protogen.Enum):
                    yield node
