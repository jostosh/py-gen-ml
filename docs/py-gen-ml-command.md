...
* `--generators TEXT`: Comma-separated subset of generators to run, e.g. `--generators=base,patch`. Defaults to every registered generator with `enabled_by_default=True` (built-in plus any third-party generators discovered via the `py_gen_ml.generators` entry-point group). Built-ins: `base`, `patch`, `sweep`, `cli_args`. Opt-in: `lancedb` (see [LanceDB schemas](guides/lancedb.md)), `bentoml` (see [BentoML services](guides/bentoml.md)), `litserve` (see [LitServe services](guides/litserve.md)), `pydantic_ai` (see [PydanticAI synthesis](guides/pydantic_ai.md)), `argilla` (see [Argilla datasets](guides/argilla.md)), `mlflow` (see [MLflow tracking](guides/mlflow.md)), `wandb` (see [Weights & Biases tracking](guides/wandb.md)).
