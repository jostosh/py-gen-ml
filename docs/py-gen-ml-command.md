...
* `--generators TEXT`: Comma-separated subset of generators to run, e.g. `--generators=base,patch`. Defaults to every registered generator (built-in plus any third-party generators discovered via the `py_gen_ml.generators` entry-point group). Built-ins: `base`, `patch`, `sweep`, `cli_args`.
